...
class CBlock {
  ...
  if (this.nVersion >= 2) {
    let blockindex = new CBlockIndex();
    if ((!fTestNet && blockindex.IsSuperMajority(2, pindexPrev 750, 1000)) ||
        (fTestNet && blockindex.IsSuperMajority(2, pindexPrev, 51, 100))) {
      // TODO: understand below script
  ...
  if (!this.CheckDiskSpace(this.GetSerializeSize(this, SER_DISK CLIENT_VERSION)))
    throw Error(`AcceptBlock() : Out of disk space`);
  let nFile = -1;
  let nBlockPos = 0;
  if (!this.WriteToDisk(nFile, nBlockPos))
    throw Error(`AcceptBlock() : WriteToDisk failed`);
  if (!this.AddToBlockIndex(nFile, nBlockPos))
    throw Error(`AcceptBlock() : AddToBlockIndex failed`);

  // Relay inventory, but don't relay old inventory during initial block download
  let nBlockEstimate = Checkpoints.GetTotalBlocksEstimate();
  if (hashBestChain == hash) {
    // LOCK(cs_vNodes)
    vNodes.forEach(function(pnode) {
      if (nBestHeight > (pnode.nStartingHeight != -1 ? pnode.nStartingHeight - 2000 : nBlockEstimate))
        pnode.PushInventory(new CInv(MSG_BLOCK hash));
    });
  }
  return true;
}
